...
| 19 | 17:30 | local Mac | GPU-sharing check reported "Chrome running" when it was not | `pgrep -f 'user-data-dir=.*chrome-profile'` matches the watcher shell whose own command line contains the pattern | Match the process name (`pgrep -x "Google Chrome"` / `ps -axo comm`), not `-f` on a pattern the checker itself carries |
