...
CLUSTER="${CLUSTER:-bonsai2-run-smoke}"; IDLE="${IDLE_MIN:-45}"; SKY="${SKY:-$HOME/.cairn-sky-venv/bin/sky}"
...
exec "$SKY" launch -c "$CLUSTER" infra/aws/smoke.sky.yaml -y -d -i "$IDLE" --down "$@"
