...
while true; do
  out=$(ssh -o BatchMode=yes -o ConnectTimeout=15 "$CLUSTER" '
    ...
    grep -HE "unit done|top-up|FAILED|died|EVALBOX DONE" ~/sky_workdir/results/'"$RES"'/evalbox*.log 2>/dev/null | sed "s|.*/evalbox||"
    echo "PROGRESS $(cat ~/sky_workdir/results/'"$RES"'/*/*/*.jsonl 2>/dev/null | wc -l) rows"' 2>&1)
  rc=$?
  if [ $rc -ne 0 ]; then
  ...
  if [ $((now - last_hb)) -ge 900 ]; then
    ...
    ssh -o BatchMode=yes -o ConnectTimeout=15 "$CLUSTER" 'cd ~/sky_workdir && rm -rf /tmp/interim && mkdir -p /tmp/interim && cp -r results/'"$RES"'/*-* /tmp/interim/ && ~/venv/bin/python scripts/score.py ~/sets /tmp/interim --speed-only' 2>&1 | grep "^|" | cut -d"|" -f2-9,10,11,13
    last_hb=$now
  fi
  sleep 60
done
