...
| 18 | 16:47 | stack box | Two units died at the start of job 2 ("server died"), both on GPU 1 | Pass B waited for "gpu1: queue empty" in pass A's log, but the log still held that line from the aborted job 1, so pass B started on GPU 1 at once; both passes used container `b2r1` and port 8091 and removed each other's server | Handoff reads only log lines written after the waiting pass starts; container names carry the pass TAG. Lost units re-queued as job 3 (`bench-stack-fix.sky.yaml`). |
| 19 | 17:30 | local Mac | GPU-sharing check reported "Chrome running" when it was not | `pgrep -f 'user-data-dir=.*chrome-profile'` matches the watcher shell whose own command line contains the pattern | Match the process name, not `-f` on a pattern the checker itself carries. Follow-up (17:40): gating on `pgrep -x "Google Chrome"` deadlocked the MLX bench for 21 min holding the GPU lock, because the user's everyday Chrome is always open. Gate on the shared lock + free memory only; never on processes the user normally runs. |
